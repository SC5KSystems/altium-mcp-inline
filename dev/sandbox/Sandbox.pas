...
begin
    ...
    try
        ...
        // Focus the built circuit sheet and zoom to fit.
        // It is an UNSAVED free document, so its DM_FullPath is a bare file name with
        // no directory - that is what distinguishes it from the project sheet of the
        // same name.
        S2 := 'Sheet7.SchDoc';
        S3 := '';
        ...
        begin
            ...
            begin
                ...
                if (Obj2.DM_FullPath = S2) then
                begin
                    S3 := Obj2.DM_FullPath;
                    SandboxLog('focusing free document ' + S3);
                    Obj2.DM_OpenAndFocusDocument;
                end;
            end;
        end;
        Sleep(1500);
        Obj1 := SchServer.GetCurrentSchDocument;
        SandboxLog('current: ' + Obj1.DocumentName + ' objectID=' + IntToStr(Obj1.ObjectID));

        // count what is on it, to confirm this is the built sheet
        I3 := 0;
        Obj2 := Obj1.SchIterator_Create;
        Obj2.AddFilter_ObjectSet(MkSet(eSchComponent));
        Obj3 := Obj2.FirstSchObject;
        while (Obj3 <> nil) do begin I3 := I3 + 1; Obj3 := Obj2.NextSchObject; end;
        Obj1.SchIterator_Destroy(Obj2);
        SandboxLog('components on this sheet = ' + IntToStr(I3));
        ...
        ResultText := '{"doc": "' + Obj1.DocumentName + '", "components": ' + IntToStr(I3) + '}';
        // === END EXPERIMENT ===
    except
